...
import { BoardTilesComponent } from '../board-tiles/board-tiles.component';
...
export class BoardComponent implements OnInit {
  ...
  createDefaultBoard( board: BoardTile[]) {
    for(let index in this.handList)
    {
      board.push( 
        {
  ...
  test: BoardTile[] = []
  boardHands = this.createDefaultBoard( this.test );

}

interface BoardTile {
  hand: string;
  raiseFrequency: string ;
  callFrequency: string ;
  foldFrequency: string ;
  boardRow: number;
  boardColumn: number;
}
